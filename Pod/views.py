...
def poddetail(request):
	if(request.session.get('user')):
		...
		if(len(res)>0 and len(res2)>0 and error==''):
			return render(request,'poddetail.html',{'res':res,'errors':'No Transaction available','res2':res2,'podid':podid,'podowner':podcreateuser,'currentuser':username,'res1':res1,'total':total,'memberamountlist':memberamountlist,'podname':podname})
		elif(len(res)>0 and len(res2)>0 and error!=''):
			return render(request,'poddetail.html',{'res':res,'error':error,'res2':res2,'podid':podid,'podowner':podcreateuser,'currentuser':username,'res1':res1,'total':total,'memberamountlist':memberamountlist,'podname':podname})
		return redirect('pods')
	return redirect('login')
...
def adduser(request):
	if(request.session.get('user')):
		...
		if(len(pod_member.objects.filter(podid=podid,useruniqueid=userid))==0):
			...
			return render(request,'searchscreen.html',{'podid':podid,'errors':'User is already Added to POD'})
		else:
			return render(request,'searchscreen.html',{'podid':podid,'errors':'Some Error occured while adding user to POD. Please contact to admin.'})
...
def addtransaction(request):
	if(request.session.get('user')):
		...
		return render(request,'showtransactionform.html',{'res1':podid,'res':exname,'podname':podname})
	return redirect('login')
...
def addtransactiondata(request):
	if(request.session.get('user')):
		...
		res= Pod_transaction(transid=transid,podmemberid=podmember[0].podmemberid,name=Expensename,useruniqueid=request.session.get('user'),podmemberName=podmember[0].name,podid=podid,amount=amount,expensetype=Expensetype,transactionmode=payment_mode,transactiondate=date)
		res.save()
		request.session['podid']=podid
...
def viewtransaction(request):
	if(request.session.get('user')):
		...
		if(res1[0].useruniqueid==request.session.get('user')):
			owner=True
		return render(request,'viewtransaction.html',{'res':res,'res1':res1,'owner':owner,'podname':podname})
	return redirect('login')
